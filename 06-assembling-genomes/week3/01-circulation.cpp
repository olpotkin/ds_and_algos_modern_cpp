...
///
/// Input: The first line contains integers n and m — the number of vertices and the number of edges, respectively. Each
///        of the following m lines specifies an edge in the format "u v l c": the edge (u, v) has a lower bound
///        l and a capacity c. (As usual, we assume that the vertices of the network are {1, 2, ... , n}.) The network
///        does not contain self-loops, but may contain parallel edges.
///
/// Output: If there exists a circulation, output YES in the first line. In each of the next m lines output the value
///         of the flow along an edge (assuming the same order of edges as in the input). If there is no circulation, output NO.
...
private:
  std::vector<Edge> edges;                 // List of all - forward and backward - edges
  std::vector<std::vector<size_t> > graph; // These adjacency lists store only indices of edges in the edges list
  std::vector<int> out, in;                // Lower bounds
  size_t last_edge;
...
public:
  ...
  void set_int_out_lower_bounds(std::vector<int> out_lbs,
                                std::vector<int> in_lbs)
  {
  ...
  void add_edge(int from, int to, int capacity, int lower_bound) {
    // Note that we first append a forward edge and then a backward edge, so all forward edges are stored
    // at even indices (starting from 0), whereas backward edges are stored at odd indices in the list edges.
...
FlowGraph read_data() {
  ...
  std::vector<int> out(vertex_count), in(vertex_count);
...
void BFS(const FlowGraph& graph,
         int s,
         int t,
         std::vector<int>& pred)
{
