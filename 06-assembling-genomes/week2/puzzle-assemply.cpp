...
/// Dataset Problem: Puzzle Assembly
///
/// Intro: In this problem, we will consider a square puzzle consisting of n-by-n square pieces, where each square piece
///        has a single color on each of its four edges. Given a set of n^2 square pieces, your task is to find a way to
///        place them in an n-by-n grid such that all adjacent edges of square pieces are of the same color.
///
/// See more details in ProgrammingAssignment2.pdf
...
struct square_puzzle
{
  ...
  void permutate_until_correct_square() {
    while (left_up.right != up[0].left || up[0].right != up[1].left || up.back().right != right_up.left)
      std::next_permutation(up.begin(), up.end());

    while(left_down.right != down[0].left || down[0].right != down[1].left || down.back().right != right_down.left)
      std::next_permutation(down.begin(), down.end());

    while(left_up.down != left[0].up || left[0].down != left[1].up || left.back().down != left_down.up)
      std::next_permutation(left.begin(), left.end());

    while(right_up.down != right[0].up || right[0].down != right[1].up || right.back().down != right_down.up)
